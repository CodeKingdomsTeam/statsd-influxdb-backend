...
function InfluxdbBackend(startupTime, config, events) {
  ...
  if (config.influxdb) {
    ...
    self.path = config.influxdb.path || self.defaultPath;
    ...
    self.tags = config.influxdb.tags || self.defaultTags;
...
InfluxdbBackend.prototype.httpPOST_v08 = function (points) {
  ...
  var options = {
    hostname: self.host,
    port: self.port,
    path: self.path + '/db/' + self.database + '/series?' + querystring.stringify(query),
    method: 'POST',
...
InfluxdbBackend.prototype.httpPOST_v09 = function (points) {
  ...
  var options = {
    hostname: self.host,
    port: self.port,
    path: self.path + '/write?' + querystring.stringify(query),
    method: 'POST',
    agent: false // Is it okay to use "undefined" here? (keep-alive)
  };
  ...
  var points_to_payload = function(points) {
    ...
    for (p in points) {
      ...
      var l = p['measurement'] + tags + ' value=' + p['fields']['value'];
      lines += l + "\n";
    }
    return lines;
  };
